...
## Expected file names
## -------------------
##   data/toks_vet_analysis.rds
##   DerivedData/reddit_dfm_analysis_corpus.rds
##
## Paths are defined by 00_project_setup.R.
###############################################################
...
## Frozen inputs created by 03_build_analysis_corpus.R.
tokens_file <- file.path(deriveddata_dir, "reddit_tokens_analysis_corpus.rds")
dfm_file    <- file.path(deriveddata_dir, "reddit_dfm_analysis_corpus.rds")
development_output_dir <- file.path(output_dir, "esfi_dictionary_development")
...
required_inputs <- c(tokens_file, dfm_file)

if (!all(file.exists(required_inputs))) {
  missing_inputs <- required_inputs[!file.exists(required_inputs)]
  stop(
    paste0(
      "Required derived-data file(s) not found:\n",
...
message("Reading tokens from: ", tokens_file)
message("Reading DFM from:    ", dfm_file)

toks_vet_analysis <- readRDS(tokens_file)
dfm_vet_analysis  <- readRDS(dfm_file)



if (!quanteda::is.tokens(toks_vet_analysis)) {
  stop("toks_vet_analysis.rds does not contain a quanteda tokens object.")
}
...
source_counts <- table(quanteda::docvars(dfm_vet_analysis, "source"))
...
dir.create(development_output_dir, recursive = TRUE, showWarnings = FALSE)
...
kwic_results <- lapply(
  kwic_patterns,
  function(pat) write_kwic_result(toks_vet_analysis, pat, development_output_dir)
)
...
write.csv(
  kwic_match_summary,
  file.path(development_output_dir, "kwic_match_summary.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(development_output_dir, "candidate_actual_matches.csv"),
  row.names = FALSE
)
...
write.csv(
  term_contribution,
  file.path(development_output_dir, "term_contribution.csv"),
  row.names = FALSE
)

##-------------------------------------------------------------
## 6. Candidate dictionary diagnostics
##-------------------------------------------------------------

## Candidate-dictionary counts are used only for development diagnostics.
## Production scores are created later from the frozen operational dictionary.

candidate_esfi_hits <- as.numeric(
  quanteda::rowSums(dfm_esfi_terms)
)

candidate_esfi_present <- candidate_esfi_hits > 0

candidate_document_prevalence <- tibble::tibble(
  documents = quanteda::ndoc(dfm_vet_analysis),
  documents_with_candidate_language = sum(candidate_esfi_present),
  percent_with_candidate_language =
    100 * mean(candidate_esfi_present),
  total_candidate_hits = sum(candidate_esfi_hits)
)

readr::write_csv(
  candidate_document_prevalence,
  file.path(
    development_output_dir,
    "candidate_document_prevalence.csv"
  )
)
...
write.csv(
  ...
  file.path(development_output_dir, "stem_contribution_complete.csv"),
  row.names = FALSE
)
...
write.csv(
  domain_contribution,
  file.path(development_output_dir, "domain_contribution.csv"),
  row.names = FALSE
)
...
write.csv(
  term_by_source,
  file.path(development_output_dir, "term_by_source.csv"),
  row.names = FALSE
)
...
write.csv(
  decision_log,
  file.path(development_output_dir, "dictionary_decision_log.csv"),
  row.names = FALSE
)

##-------------------------------------------------------------
## 10. Freeze and apply the operational ESFI dictionary
##-------------------------------------------------------------
...
actual_esfi_matches <- quanteda::featnames(dfm_esfi)

message(
  "Operational ESFI dictionary matched ",
  length(actual_esfi_matches),
  " distinct DFM features:"
)
print(actual_esfi_matches)

esfi_hits <- as.numeric(quanteda::rowSums(dfm_esfi))
esfi_present <- esfi_hits > 0

dfm_vet_analysis_with_esfi <- dfm_vet_analysis
...
document_prevalence <- tibble::tibble(
  documents = quanteda::ndoc(dfm_vet_analysis_with_esfi),
  documents_with_esfi = sum(esfi_present),
  percent_with_esfi = 100 * mean(esfi_present),
  total_esfi_hits = sum(esfi_hits)
)

esfi_summary_by_source <- tibble::tibble(
  source = quanteda::docvars(
    dfm_vet_analysis_with_esfi,
    "source"
  ),
  esfi_hits = esfi_hits,
  ...
  dplyr::summarise(
    ...
    mean_hits_positive_documents = ifelse(
      sum(esfi_present) > 0,
      mean(esfi_hits[esfi_present]),
      NA_real_
    ),
    median_hits_positive_documents = ifelse(
      sum(esfi_present) > 0,
...
readr::write_csv(
  tibble::tibble(
    matched_feature = actual_esfi_matches
  ),
  file.path(
    development_output_dir,
    "esfi_actual_matches.csv"
  )
)
...
saveRDS(
  ...
  file.path(
    deriveddata_dir,
    "reddit_analysis_metadata_with_esfi.rds"
  )
)
...
readr::write_csv(
  ...
  file.path(
    deriveddata_dir,
    "reddit_analysis_metadata_with_esfi.csv"
  )
)
...
operational_pattern_summary <- tibble(
...
write.csv(
  ...
  file.path(development_output_dir, "operational_pattern_summary.csv"),
  row.names = FALSE
)

## Known frozen ESFI targets
expected_matches <- c(
  "denied",
  "wait",
  "claim",
...
stopifnot(
  length(actual_esfi_matches) == 11L,
  sum(esfi_hits) == 428L,
  setequal(actual_esfi_matches, expected_matches)
)
...
esfi_development_bundle <- list(
  ...
  operational_dictionary = esfi_dictionary,
  kwic_match_summary = kwic_match_summary,
  actual_candidate_matches = actual_candidate_matches,
  actual_esfi_matches = actual_esfi_matches,
  term_contribution = term_contribution,
  stem_contribution = stem_contribution_complete,
  domain_contribution = domain_contribution,
  term_by_source = term_by_source,
  decision_log = decision_log,
  operational_pattern_summary = operational_pattern_summary
)

saveRDS(
  esfi_development_bundle,
  file.path(development_output_dir, "esfi_dictionary_development_bundle.rds")
)

message("")
message("04_develop_esfi_dictionary.R completed successfully.")
message("Analysis documents scored: ", quanteda::ndoc(dfm_vet_analysis_with_esfi))
message("Distinct matched features: ", length(actual_esfi_matches))
message("Total ESFI hits: ", sum(esfi_hits))
message("Documents with ESFI language: ", sum(esfi_present))
